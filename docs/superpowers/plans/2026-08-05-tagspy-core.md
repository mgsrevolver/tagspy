...
| Plan | Scope |
| --- | --- |
| 2 | `naming-collision`, `placeholder-param`, `cross-platform-gap`, `utm-loss`, `consent-suppression`, `push-before-init`, `ecommerce-not-cleared`. Also from live capture: `event-name-length` (GA4 truncates at 40 chars — observed on shop.merch.google) and `debug-mode-in-prod` (`ep.debug_mode=True` observed in production). **Carried review deferrals:** (a) `decodeConsent` collapses `gcs=G1--` (CMP unset) into `null`, indistinguishable from "no consent signaling" — the consent-suppression rule needs the adapter to decode dashes as `'unset'` first. (b) dataLayer adapter drops the payload of `gtag('set', {...})` (object at args[1], not args[2]) and of sparse arguments objects (`{0:'config', 2:{...}}`) — harden before Plan 2's dataLayer rules rely on gtag-call params. **From the final whole-branch review:** (c) consent diverges two ways between adapters — GA4 normalizes to `consent:{ads,analytics}` on the event; dataLayer leaves `consent:null` and puts raw `{ad_storage,...}` in a synthetic `gtag.consent.*` event's params — unify before the consent-suppression rule. (d) `dataLayer.push({ecommerce:null})` (the canonical ecommerce CLEAR) produces no event at all, so `ecommerce-not-cleared` cannot see clears — **this blocks that rule; harden the adapter first.** (e) `revenue-without-currency` reads flat `params.value` only; GTM's nested `ecommerce:{value,currency}` shape is a verified false negative, and the rule double-fires per channel (ga4 + datalayer) for one logical event. (f) `duplicate-event`'s dedupe key omits `pageUrl` — two page_views on different URLs 300ms apart false-positive during an automated funnel walk. (g) `order` is not chronological across sources (dataLayer events append after network events before renumbering) — safe within one source, wrong for cross-source rules. (h) Decide whether dataLayer is an evidence channel or a first-class rule target — finding counts double per logical event as adapters multiply. |
| 3 | Meta, Google Ads, TikTok, LinkedIn adapters (registry and rules already accept them unchanged). **Adapter checklist from reviews:** every adapter MUST construct events via `tagEvent()` — rules assume the factory's `params: {}` guarantee and will throw on a hand-built event with `params: undefined`; `matches()` may throw (registry guards it), `decode()` may throw (registry records it). Real captured `/ccm/collect` and `googleads` URLs are in the GA4 fixture as starting material. **From the final review:** the GA4 adapter deliberately omits `/j/collect`, so the live `UA-54516992-1` hit in the storefront fixture decodes to nothing and network-side dead-UA properties are invisible — decide whether legacy UA transport gets its own adapter here. Checklist addition: rules must not mutate `params` (registry shares it by reference with adapter output). |
| 4 | `tracking-plan.yml` loader, assertions, `once_per` scopes, waivers, exit codes, spec bootstrapping. **Waiver-design input from review:** `duplicate-event`'s waiveKey (`rule:platform:event`) is deliberately coarser than one finding — waiving it silences all duplicates for that event; decide per-transaction granularity here. Also: burst of N repeats currently yields N−1 pairwise findings; consider burst-collapse in Plan 2. `ctx.duplicateWindowMs` exists but nothing plumbs it — the plan loader here is its intended caller. |
